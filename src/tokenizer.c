...
char **tokenize(char* str)
{
  ...
  char **tokens =(char**)malloc((numWords+1)* sizeof(char *)); //char arr allocates memory + '/0'
  int i = 0;
  while(i<numWords){ //loop copys each word from string into the array leaving space for '0'
    str = word_start(str); //stores 1st char of string
    int strLen = word_terminator(str) - str; //stores len of word in str
    tokens[i] = copy_str(str,strLen);
